...
function idCheck() {
	...
	// => 영문과 숫자로만 입력했는지 : id 에서 영문과 숫자를 모두 '' 로 변경했을때 length 가 0 이면 OK   
		...
		return false;
	// => test(검사대상문자) 메서드 활용 (불규칙함 => replace로)
...
function pwCheck() {
	...
	//}else if(!special.test(pw) ) {
	 ...
	 document.getElementById('pMessage').innerHTML='password는 영문,숫자,특수숫자만 가능합니다.';
		return false;
	// => 특수문자는 반드시 포함 replace 적용
	}else if(pw.replace(/[!-*.@]/gi,'').length >= pw.length) {
		document.getElementById('pMessage').innerHTML='password는 특수문자가 반드시 포함 되어야 합니다.';
		return false;
	}else {
...
function nmCheck() {
	...
	let special = /[a-z.가-힣]/gi;
	if (name.length<2 ) {
		document.getElementById('nMessage').innerHTML='name는 2글자 이상 입니다.';
		return false;
	// => 영문과 숫자로만 입력했는지 : id 에서 영문과 숫자를 모두 '' 로 변경했을때 length 가 0 이면 OK   
	}else if( name.replace(special,'').length>0) {
		document.getElementById('nMessage').innerHTML='name는 영문과 한글만 입력 가능합니다.';
		return false;
...
function agCheck() {
	...
	if (age.replace(/[^0-9]/, '').length < age.length ||
		...
		document.getElementById('aMessage').innerHTML='age는 정수만 입력 하세요~~';
		return false;
...
function poCheck() {
	...
	// => 숫자 아닌값이 있는지 호가인, Number.isNaN(...) 적용
	// => 단, 소숫점은 허용
	// 	(비교값으로 소숫점을 사용하기위해 "\." 으로 표기한다.)
	if (point.replace(/[^0-9.\.]/g,'').length < point.length ||
		Number.isNaN(parseFloat(point))	) {
		document.getElementById('oMessage').innerHTML='point는 정수 또는 실수만 입력 하세요~~';
		return true;
